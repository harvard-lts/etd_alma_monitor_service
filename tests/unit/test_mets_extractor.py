...
def test_mets_extractor():
    mets_test_file = os.path.join(os.getenv("ALMA_DATA_DIR"), "mets.xml")
    mets_extractor = MetsExtractor(mets_test_file)
    amdid_thesis = mets_extractor.get_amdid_and_mimetype("ES 100 Final Thesis PDF - Liam Nuttall.pdf")
    amdid_license = mets_extractor.get_amdid_and_mimetype("Harvard_IR_License_-_LAA_for_ETDs_(2020).pdf")
